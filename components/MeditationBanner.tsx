...
const Waveform: React.FC<{ className?: string }> = ({ className = '' }) => (
  <div className={`flex items-end justify-between gap-[2px] h-6 ${className}`} aria-hidden="true">
...
const MeditationBanner: React.FC = () => {
  ...
  return (
    <div className="relative z-20 w-full max-w-sm mx-auto mb-8 -translate-y-[80px] animate-fade-in-up [animation-delay:0.1s] [animation-fill-mode:both]">
      ...
      {/* Druga apla: kod QR obok, po prawej stronie (tylko szerokie ekrany) */}
      <div className={`hidden xl:block absolute top-0 left-full ml-4 w-[340px] rounded-4xl border border-warm-300/15 bg-warm-50/30 backdrop-blur-md shadow-xl shadow-warm-50/20 overflow-hidden transition-all duration-500 ${expanded ? 'opacity-0 translate-x-4 pointer-events-none' : 'opacity-100 translate-x-0'}`}>
        <div className="flex items-center justify-center gap-5 px-5 py-4 min-h-[156px]">
