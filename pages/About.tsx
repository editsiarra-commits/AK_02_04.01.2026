...
const About: React.FC<SectionProps> = ({ id }) => {
  return (
    <section id={id} className="relative py-24 bg-surface-100 scroll-mt-20 overflow-hidden">
      <style>{`
        @keyframes scroll {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <h2 className="font-serif text-4xl md:text-5xl tracking-wide mb-6 text-navy-900">O Mnie</h2>
          <p className="font-sans text-gray-400 max-w-2xl mx-auto tracking-wider uppercase text-sm">Poznaj osobę stojącą za głosem</p>
        </div>

        <div className="flex flex-col md:flex-row gap-16 items-start mb-20">
          <div className="w-full md:w-5/12">
            <div className="aspect-[3/4] bg-surface-200 overflow-hidden shadow-xl rounded-xl relative group sticky top-24">
              <div className="absolute inset-0 border border-white/30 z-10 m-4 rounded-lg transition-all duration-500 group-hover:m-0"></div>
              <img
          ...
          <div className="w-full md:w-7/12 space-y-8">
            <h3 className="font-serif text-3xl text-coral-500 italic">Moja Podróż</h3>
            ...
            <div className="py-8">
              <blockquote className="border-l-2 border-coral-500 pl-8 italic text-navy-900 text-xl font-serif leading-relaxed">
        ...
        <div className="mb-20">
          <div className="flex items-center space-x-4 mb-10">
            <div className="h-px bg-surface-300 flex-grow"></div>
            <h3 className="font-serif text-2xl text-navy-900 uppercase tracking-widest text-center px-4">Kwalifikacje i Edukacja</h3>
            <div className="h-px bg-surface-300 flex-grow"></div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {credentials.map((cred, index) => (
              <div key={index} className="bg-white p-6 border border-surface-300 rounded-xl hover:border-coral-300 transition-colors flex items-start space-x-4 group hover:shadow-lg">
                <div className="flex-shrink-0 w-12 h-12 bg-coral-50 rounded-xl flex items-center justify-center text-coral-500 group-hover:bg-coral-100 transition-colors">
                  <cred.icon size={24} strokeWidth={1.5} />
                </div>
                <div>
                  <h4 className="text-navy-900 font-serif text-xl mb-1">{cred.title}</h4>
                  <p className="text-coral-500 text-xs font-bold uppercase tracking-wider mb-2">{cred.org}</p>
                  <p className="text-gray-500 text-sm font-light leading-relaxed">{cred.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Logo Slider Section */}
        <div className="border-t border-surface-300 pt-16">
          <p className="text-center text-xs font-bold uppercase tracking-[0.2em] text-coral-500 mb-10">       </p>
          <div className="relative w-full overflow-hidden group/slider">
            {/* Fade edges */}
            <div className="absolute top-0 left-0 h-full w-24 bg-gradient-to-r from-surface-100 to-transparent z-10 pointer-events-none"></div>
            <div className="absolute top-0 right-0 h-full w-24 bg-gradient-to-l from-surface-100 to-transparent z-10 pointer-events-none"></div>

            <div className="flex animate-scroll w-max">
              {[...logos, ...logos, ...logos].map((logo, index) => (
                <div key={index} className="mx-6 md:mx-10 w-48 h-24 flex items-center justify-center opacity-50 hover:opacity-100 transition-opacity duration-300 cursor-pointer">
