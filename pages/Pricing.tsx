...
const Pricing: React.FC<PricingProps> = ({ id }) => {
  ...
  // We mock useNavigate for prototype
    ...
    <div className="bg-warm-900 text-warm-200 min-h-screen">
      ...
      <section className="relative py-16 md:py-20 border-b border-warm-800 overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center pt-8">
          <p className="text-coffee-400 font-sans text-sm md:text-xl font-bold uppercase tracking-[0.2em] md:tracking-[0.3em] mb-4">
            Inwestycja w Twoją Zmianę
          </p>
          <h1 className="font-serif text-4xl md:text-6xl text-warm-100 mb-4">
            Cennik
          </h1>
          <div className="text-lg md:text-2xl text-warm-300 font-serif italic max-w-3xl mx-auto space-y-4 px-4">
            <p>To nie są po prostu „sesje”.</p>
            <p className="text-[#32303b] text-base md:text-xl">To starannie zaprojektowane przestrzenie, w których możesz przestać walczyć ze sobą i zacząć żyć w zgodzie z tym, kim naprawdę jesteś.</p>
            <p className="text-base md:text-xl">Jeśli czujesz, że to może być Twój moment – napisz lub zadzwoń.</p>
      ...
      <div className="container mx-auto px-4 py-0">
        <div className="max-w-6xl mx-auto py-10">
          <div className="mb-8 pl-0 sm:pl-12 text-center sm:text-left">
          ...
          <div className="text-left mx-0 sm:mx-16 md:mx-20 lg:mx-28 mb-10 px-4 sm:px-0">
            <p className="text-warm-300 text-base md:text-lg font-light mb-6">
            ...
            <ul className="space-y-4 mb-8">
              <li className="flex items-start text-warm-300">
                ...
                <span className="text-sm md:text-base">precyzyjnie nazwiemy to, co naprawdę chcecie zmienić / uzdrowić / rozwinąć</span>
              </li>
              <li className="flex items-start text-warm-300">
                <Check size={20} className="text-coffee-400 mr-3 mt-0.5 shrink-0" /> 
                <span className="text-sm md:text-base">poczujemy, która ścieżka będzie dla Ciebie najbardziej precyzyjna i bezpieczna</span>
              </li>
              <li className="flex items-start text-warm-300">
                <Check size={20} className="text-coffee-400 mr-3 mt-0.5 shrink-0" /> 
                <span className="text-sm md:text-base">ustalimy rytm i terminy, które będą Ci służyć.</span>
              </li>
            </ul>
          ...
          <div className="w-full relative px-2 sm:px-0">
            <div className="max-w-6xl mx-auto relative">
              ...
              <button 
                onClick={prevSlide}
                className="absolute left-2 sm:-ml-8 top-[35%] sm:top-1/2 -translate-y-1/2 z-20 p-2 sm:p-4 rounded-full bg-warm-950/90 backdrop-blur-sm border border-warm-700 text-coffee-300 shadow-xl shadow-black/40 hover:bg-warm-900 hover:text-coffee-200 hover:border-coffee-500 hover:scale-105 transition-all focus:outline-none group"
              ...
              <button 
                onClick={nextSlide}
                className="absolute right-2 sm:-mr-8 top-[35%] sm:top-1/2 -translate-y-1/2 z-20 p-2 sm:p-4 rounded-full bg-warm-950/90 backdrop-blur-sm border border-warm-700 text-coffee-300 shadow-xl shadow-black/40 hover:bg-warm-900 hover:text-coffee-200 hover:border-coffee-500 hover:scale-105 transition-all focus:outline-none group"
              ...
              <div className="overflow-hidden sm:mx-0">
                ...
                >
                  {pricingItems.map((item, index) => (
                    <div key={item.id} className="w-full shrink-0 pb-8 sm:pb-16 md:pb-20 pt-4 sm:pt-[50px] px-2 sm:px-16 md:px-20 lg:px-28 h-[650px]">
                      <div className="bg-warm-950 rounded-2xl sm:rounded-[40px] overflow-hidden border border-warm-800/80 flex flex-col md:flex-row h-[550px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] hover:border-warm-700 transition-all duration-500 relative">
                        
                        {/* Left Column: Title & Price */}
                        <div className="p-6 sm:p-10 md:p-14 md:w-5/12 bg-warm-900 flex flex-col justify-between relative overflow-hidden border-b md:border-b-0 md:border-r border-warm-800">
                          
                          <div className="relative z-10 text-center md:text-left">
                            <div className="text-coffee-400 font-sans text-[10px] sm:text-xs uppercase tracking-[0.2em] mb-4 sm:mb-8">
                              0{index + 1} — 0{pricingItems.length}
                            </div>
                            
                            <h3 className="font-serif text-2xl sm:text-3xl md:text-4xl text-coffee-600 leading-snug mb-2 sm:mb-4 font-medium">
                              {item.title}
                            </h3>
                            <p className="text-warm-400 text-sm sm:text-lg font-light tracking-wide">
                              {item.subtitle}
                            </p>
                          </div>
                          
                          <div className="mt-8 sm:mt-14 mb-4 md:mb-0 relative z-10 text-center md:text-left">
                            <div className="flex items-baseline justify-center md:justify-start font-serif tracking-tight">
                              <span className="text-4xl sm:text-5xl md:text-6xl text-warm-100 mr-2">{item.price.split(' ')[0]}</span>
                              <span className="text-2xl sm:text-3xl md:text-4xl text-black font-light">{item.price.split(' ')[1]}</span>
                            </div>
                            <div className="text-warm-500 mt-2 sm:mt-4 font-light text-xs sm:text-sm tracking-widest uppercase">
                              {item.duration}
                            </div>
                          </div>
                        ...
                        <div className="p-6 sm:p-10 md:p-14 md:w-7/12 flex flex-col justify-center bg-transparent h-[550px]">
                          <p className="text-warm-300 text-sm sm:text-lg font-light leading-relaxed mb-6 sm:mb-[25px]">
                            {item.description}
                          </p>
                          
                          <div className="space-y-4 sm:space-y-6">
                            <h4 className="font-sans text-[13px] sm:text-[15px] font-normal transition-all text-coffee-400 text-left">
                              {item.targetAudience}
                            </h4>
                            <ul className="space-y-3 sm:space-y-5">
                              {item.details.map((detail, i) => (
                                <li key={i} className="flex items-start text-warm-300">
                                  <div className="mt-1.5 sm:mt-2.5 mr-3 sm:mr-5 shrink-0 w-1.5 h-1.5 rounded-full bg-coffee-500/60"></div>
                                  <span className="font-light leading-relaxed text-xs sm:text-[15px]">{detail}</span>
                                </li>
                              ))}
                            </ul>
                          </div>
                          ...
                          <div className="mt-8 sm:mt-12 md:hidden">
                            <a href="#contact" onClick={handleContactClick} className="block w-full py-3 sm:py-4 text-center bg-coffee-600 text-white font-sans hover:bg-coffee-500 transition-all duration-300 uppercase text-[10px] sm:text-xs tracking-widest rounded-full">
                              Zarezerwuj Termin
                            </a>
                          </div>
                        </div>
              ...
              <div className="flex justify-center space-x-2 sm:space-x-3 mt-2 sm:mt-6 mb-4">
                {pricingItems.map((_, idx) => (
                  <button
                    key={idx}
                    onClick={() => setCurrentSlide(idx)}
                    className={`h-1.5 rounded-full transition-all duration-500 ease-out cursor-pointer ${
                      currentSlide === idx ? 'bg-coffee-400 w-6 sm:w-10 shadow-[0_0_8px_rgba(154,115,82,0.6)]' : 'bg-warm-700 w-2 sm:w-3 hover:bg-coffee-500 hover:w-3 sm:hover:w-5'
                    }`}
